...
class prior_covid:
    ...
    def lpdf(theta):
        return (sps.norm.logpdf(theta[:, 0], 2.5, 1.5) +
                sps.norm.logpdf(theta[:, 1], 4.0, 1.5) + 
                sps.norm.logpdf(theta[:, 2], 4.0, 1.5) + 
                sps.norm.logpdf(theta[:, 3], 1.875, 1.1) + 
    ...
    def rnd(n):
        return np.vstack((sps.norm.rvs(2.5, 1.5, size=n),
                          sps.norm.rvs(4.0, 1.5, size=n),
                          sps.norm.rvs(4.0, 1.5, size=n),
                          sps.norm.rvs(1.875, 1.1, size=n),
...
cls_params = prior_covid.rnd(10000)
