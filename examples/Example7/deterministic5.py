...
par_out = param_values_rnd[np.logical_or.reduce((func_eval_rnd[:, 100] <= 100, func_eval_rnd[:, 100] > 1000)), :]
...
y = np.zeros(len(cls_params))
y[np.logical_and.reduce((pred_mean_1.T[:, 100] > 100, pred_mean_1.T[:, 100] < 1000))] = 1

# Create a balanced data set
X_0 = cls_params[y == 0][0:500]
X_0test = cls_params[y == 0][500:1000]
y_0 = y[y == 0][0:500]
y_0test = y[y == 0][500:1000]

X_1 = cls_params[y == 1][0:500]
X_1test = cls_params[y == 1][500:1000]
y_1 = y[y == 1][0:500]
y_1test = y[y == 1][500:1000]
...
cal_f = calibrator(emu = emulator_1,
                   y = real_data,
                   x = x,
...
#breakpoint()
cal_f_ml = calibrator(emu = emulator_1,
                      y = real_data,
                      x = x,
                      thetaprior = prior_covid,
                      method = 'mlbayes',
