...
method = ['EMGP', 'colGP', 'PCGPwM', 'GPy', 'PCGP_KNN', 'PCGP_benchmark', 'PCGP_BR']

combs = np.array(())
base = np.array(np.meshgrid(n, function, failrandom, failfrac, method)).T.reshape(-1, 5)
for i in range(10):
    rep = np.random.randint(1, 50000, size=60)
    reps = np.tile(rep, 7)
...
np.savetxt('params0MAR.txt', combs[:2100], fmt='%s', delimiter='\t')
np.savetxt('params1MAR.txt', combs[2100:4200], fmt='%s', delimiter='\t')
