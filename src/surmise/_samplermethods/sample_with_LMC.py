...
def sample_with_LMC(thetaprior, loglik, draw_func, options=None):
    '''
    ...
    '''
    VALID_OPTIONS = {"numsamp", "theta0", "verbose"}
    ...
    # Assume that if they gave us the grad of the prior then they want to use it
    # and that the loglik also computes the gradient.
    use_grad=("lpdf_grad" in dir(thetaprior))
    logpost_func = construct_log_joint_posterior(
        thetaprior, loglik, use_grad=use_grad
    )
    ...
    # Test
    # TODO: This could probably be done better when we update.
    testout = logpost_func(theta0[0:2, :])
    if use_grad:
        assert isinstance(testout, tuple)
        if len(testout) != 2:
